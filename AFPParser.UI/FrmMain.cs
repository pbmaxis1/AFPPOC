            ...
            if (result == DialogResult.OK)
            {
                try
                {
                    ...
                    if (afpFile.LoadData(dialog.FileName, true))
                    {
                        ...
                        int totalPages = printParser.pageContainers.Count;
                        //var test = printParser.pageContainers[0].Structures;
                        byte[] testBytes = new byte[64];
                        
                        //Console.WriteLine(testBytes);

                        //System.IO.File.WriteAllBytes(@"G:\Raghu-PC\ASP.NET\AFP Parse\AFPParser\1stPage.AFP", testBytes);
                        lblPrintTotalPages.Text =Convert.ToString(totalPages);
